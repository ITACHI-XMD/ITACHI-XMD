...
const antistatusmention = require('./commands/antistatusmention');
const { isStatusMention, isEnabled: isAntiStatusMentionEnabled, handleAntistatus } = require('./commands/antistatusmention');
...
const { pollCommand, gcreateCommand, joinCommand, leaveCommand, lockCommand, unlockCommand, kickallCommand, vcfCommand, tagadminCommand, acceptallCommand, rejectallCommand } = require('./commands/groupeplus');
...
async function handleMessages(sock, messageUpdate, printLog) {
    try {
        ...
        if (isGroup) {
            ...
            try {
                const deleted = await handleAntistatus(sock, chatId, senderId, message);
                if (deleted) return;
            } catch (e) { /* non critique */ }
        }
        ...
        switch (true) {
            ...
            case userMessage.startsWith('.antistatusmention'): {
                ...
                await antistatusmention(sock, chatId, senderId, asmArgs, message);
                break;
            }
            ...
            // ══════════════════════════════════════════
            // 🥷 Commandes supplémentaires v2.0
            // ══════════════════════════════════════════
